...
namespace baseSys.Audio.Method {
    public class PlayMethod {
        ...
        public float PlayTime(string name)
        {
            ...
            {
                foreach (GameObject go in _nowPlayer)
                    if (go.name == name)
                        timer = go.GetComponent<AudioSource>().clip.length;
            }
            else
            {
        ...
        public void NextPlay(string name,float time)
        {
            //如果播放清單有
            if (_list.ContainsKey(name))
            {
        ...
        void nextPlay(string name,float _time)
        {
            int playerCount =
                _nowPlayer.Count;
            //取得物件
